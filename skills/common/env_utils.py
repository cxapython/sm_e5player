...
"""
环境变量加载工具模块
统一处理 .env 文件加载，遵循 canghe-image-gen 的加载顺序
"""
...
def load_env() -> None:
    ...
    2. <cwd>/.canghe-skills/.env (当前工作目录)
    3. ~/.canghe-skills/.env (用户主目录)
    """
    home = Path.home()
    cwd = Path.cwd()

    # 从低优先级开始加载，高优先级会覆盖低优先级
    home_env = load_env_file(home / ".canghe-skills" / ".env")
    cwd_env = load_env_file(cwd / ".canghe-skills" / ".env")
...
def require_env_key(key_name: str, alias_names: Optional[List[str]] = None, error_message: Optional[str] = None) -> str:
    ...
    if not val:
        ...
        print(f"   1. 复制 .canghe-skills/.env.example 到 .canghe-skills/.env 并填写", file=sys.stderr)
