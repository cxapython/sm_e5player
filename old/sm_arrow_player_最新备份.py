...
class ArrowPlayer:
    ...
    def draw(self):
        ...
        # 绘制判定统计（右侧）
        self._draw_judge_stats(track_start_x, track_total_w)
        # 绘制tick匹配疑点提示
    ...
    def _draw_top_info(self, y1: int, y2: int, y3: int):
        ...
        # 文本2：时间+播放状态+分数+连击
        text2 = f"时间：{format_seconds(cur_sec)} / {format_seconds(self.total_chart_sec)}   播放：{'是' if self.is_playing else '否'}   分数：{self.score}   连击：{self.combo}"
        # 文本3：参数信息
    ...
    def _draw_judge_display(self, judge_y: int):
        ...
        # 绘制判定文字（大字体）
        judge_font = safe_load_chinese_font(36)
        text_surf = judge_font.render(self.judge_display_text, True, color)
        text_rect = text_surf.get_rect(center=(self.window_w // 2, judge_y + 80))
        self.screen.blit(text_surf, text_rect)

    def _draw_judge_stats(self, track_start_x: int, track_total_w: int):
        """绘制判定统计（右侧面板）"""
        # 右侧面板位置
        panel_x = track_start_x + track_total_w + 20
        panel_y = 140
        # 标题
        title_surf = self.small_font.render("判定统计", True, (200, 200, 210))
        ...
        stats = [
            ...
            (f"BAD:     {self.judge_count['BAD']}", (255, 80, 80)),
            (f"MISS:    {self.judge_count['MISS']}", (150, 150, 150)),
        ]
        for i, (text, color) in enumerate(stats):
            text_surf = self.small_font.render(text, True, color)
            self.screen.blit(text_surf, (panel_x, panel_y + 25 + i * 22))
        # 最大连击
        max_combo_surf = self.small_font.render(f"Max Combo: {self.max_combo}", True, (255, 200, 100))
        self.screen.blit(max_combo_surf, (panel_x, panel_y + 25 + 4 * 22 + 10))
